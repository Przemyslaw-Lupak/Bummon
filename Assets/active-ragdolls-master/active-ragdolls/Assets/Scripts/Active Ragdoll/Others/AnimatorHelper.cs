...
public class AnimatorHelper : NetworkBehaviour {
    ...
    private void Update() {
        // IK weight updates should happen on server where physics runs
        if(!IsServer) return;
        UpdateIKTransitions();
    }
    ...
    private void OnAnimatorIK(int layerIndex) {
        // IK calculations happen on the server
        if (!IsServer) return;
        ...
        // Left arm
        if (LeftHandTarget != null && LeftHandHint != null) {
        ...
        // Right arm
        if (RightHandTarget != null && RightHandHint != null) {
