...
public class PhysicsModule : NetworkBehaviour {
    ...
    private void FixedUpdate() {
        // ALL physics runs on server only
        if(!IsServer) return;
        ...
        switch (_balanceMode) {
            ...
            case BALANCE_MODE.FREEZE_ROTATIONS:
                ...
                _playerRagdoll.GetPhysicalTorsoRigidbody().MoveRotation(smoothedRot.normalized);
                break;

            case BALANCE_MODE.STABILIZER_JOINT:
                _stabilizerRigidbody.MovePosition(_playerRagdoll.GetPhysicalTorsoRigidbody().position);
                _stabilizerRigidbody.MoveRotation(_targetRotation.normalized);
                break;
    ...
    private void UpdateTargetRotation() {
        // Target rotation should be updated on server (where physics runs)
        // But TargetDirection comes from owner's input/camera
        if (TargetDirection != Vector3.zero && TargetDirection.sqrMagnitude > 0.001f)
            _targetRotation = Quaternion.LookRotation(TargetDirection, Vector3.up).normalized;
        else
    ...
    private void ApplyCustomDrag() {
        if (_playerRagdoll.GetPhysicalTorsoRigidbody().isKinematic) {
            Debug.LogError($"[PhysicsModule] PhysicalTorso is KINEMATIC! This should only happen on clients.");
            return;
        }
    ...
    public void ManualTorqueInput(Vector2 torqueInput) {
        // Input comes from owner, store it for server to use
